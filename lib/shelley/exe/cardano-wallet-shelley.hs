...
import Cardano.Wallet.Shelley.Network
    ( LaunchConfig (..), UseRunningOrLaunch (..), mainnetConnectionParams )
  ...
  where
    cmd = fmap exec $ LaunchArgs
        ...
        <*> (JormungandrArgs
            <$> genesisBlockOption
            <*> extraArguments)
    exec (LaunchArgs hostPreference listen _nodePort mStateDir logCfg sTolerance verbosity jArgs) = do
...
cmdServe = command "serve" $ info (helper <*> cmd) $ mempty
  ...
  where
    ...
    exec
        :: ServeArgs
        -> IO ()
    exec (ServeArgs hostPreference listen _nodePort databaseDir logCfg sTolerance verbosity _block0h) = do
        let minSeverity = verbosityToMinSeverity verbosity
        withLogging logCfg minSeverity $ \(cfg, tr) -> do
            -- TODO: Parse from cmd args
            let cp = mainnetConnectionParams
